...
;
	...
	private:
		...
		void InitializeComponent(void)
		{
			...
			this->comboBoxQuality->Items->Add("Baixa");
			this->comboBoxQuality->Items->Add("Média");
			this->comboBoxQuality->Items->Add("Alta");
			...
			this->comboBoxSky->Items->Add("Céu Aberto");
			this->comboBoxSky->Items->Add("Parcialmente Nublado");
			this->comboBoxSky->Items->Add("Nublado");
										...
										if (DateHourIsValid())
										{
											if (textBoxLatitude->Text!= "")
											{
												if (textBoxLongitude->Text != "")
												{
													...
													FormsImage^ fi = gcnew FormsImage(img);
													fi->ShowDialog();
		...
		/*XYZ^ GetPositionSun() 
		{
			/*^ positionSun = gcnew PSA(GetLatitude(), GetLongitude(), day, month, year, hour, min);
