...
{
	RibbonPanel^ ribbonPanel = application->CreateRibbonPanel("Renderizador");
	// Create a push button to trigger a command add it to the ribbon panel.
	System::String^ thisAssemblyPath = Assembly::GetExecutingAssembly()->Location;
	PushButtonData^ buttonData = gcnew PushButtonData("cmdPixeR", "PixeR", thisAssemblyPath, "Main.Command");
