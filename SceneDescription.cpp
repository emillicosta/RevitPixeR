...
Bitmap^ SceneDescription::GetImage()
{
	...
	if (sun->Z > 0) {
		...
		lights.push_back(new PointLight(origin, rgb(1, 1, 0.85)));
	}
	else
	{
		al = rgb(0,0,0);
		...
		else
		{
			...
			if (nullptr != geoInstance)
			{
				...
				for each(GeometryObject^ o2 in symbolGeo)
				{
					...
					if (nullptr != s)
					{
						GraphicsStyle^ gStyle = dynamic_cast<GraphicsStyle^>(doc->GetElement(s->GraphicsStyleId));
						if (gStyle != nullptr)
						{
							if (gStyle->GraphicsStyleCategory->Id->IntegerValue == (int)(BuiltInCategory::OST_LightingFixtureSource)) 
							{
								...
								for each (Face ^ geomFace in s->Faces)
								{
									...
									TaskDialog::Show("numero de triangulos de cada face","" + mesh->NumTriangles);
								}*/
								continue;
							}
						}
...
MyMaterial* SceneDescription::GetMyMaterial(Material^ matRevit, ElementId^ id)
{
	for (int i = 0; i < elem_light->Count; i++) {
		if (id == elem_light[i] && matRevit->MaterialClass == "Vidro")
	...
	if (matRevit != nullptr) {
		...
		if (path != nullptr)
		{
			...
			if (s != "") {
				...
				TaskDialog::Show("Connected bitmap", path);*/
		...
		if (matRevit->MaterialClass == "Metal") {
			int m = 2;
			if(FindMetalFinish(matRevit) == "0" || FindMetalFinish(matRevit) == "1" || FindMetalFinish(matRevit) == "3")
				m = System::Convert::ToInt32(FindMetalFinish(matRevit));
		...
		//TaskDialog::Show("nome do material", matRevit->Name);
		if (matRevit->Name == "Vidro, Branco, Alta luminosidade") {
			return new LightMaterial(new ConstantTexture(kd));
		}

		if (matRevit->MaterialClass == "Vidro") {
			SolidGlass^ vid = dynamic_cast<SolidGlass^>(matRevit);
			if (vid != nullptr)
				TaskDialog::Show("Reflectancia", vid->SolidglassRefractionIor);
	...
	return new DifuseMaterial(new ConstantTexture(rgb(0.5,0.5,0.5)));
}
...
Background* SceneDescription::GetBackground()
{
	if (sun->Z > 0) {
		int distancia = 200;
		rgb lower_left = rgb(1, 0.5 + sun->Z/(2*distancia), sun->Z/ distancia);
		rgb lower_right = rgb(1.f, 0.f, 0.f);
		rgb top_left = rgb(0, sun->Z / distancia, 0.5+ sun->Z / (2*distancia));
	...
	{
		return new Background(rgb(0,0,0), rgb(0, 0, 0), rgb(0, 0, 0), rgb(0, 0, 0));
	}
}
